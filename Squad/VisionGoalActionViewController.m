//
//  VisionGoalActionViewController.m
//  Squad
//
...
@interface VisionGoalActionViewController () {      //ah ac     //ah ac2
    ...
    IBOutletCollection(UIView) NSArray *dateCollection;
    IBOutletCollection(UIView) NSArray *goalCollection;
    IBOutlet UIButton *dateExpandButton;
    IBOutlet UIButton *goalExpandButton;
    IBOutlet UIButton *goalArrButton;
    IBOutlet UIButton *dateArrButton;
    IBOutlet UIButton *selectDateBtn;
    IBOutletCollection(UIButton) NSArray *goalDatetickBtn;
    IBOutletCollection(UIButton) NSArray *resultCollectionBtn;
    IBOutlet UILabel *nodataLabel;
    IBOutlet UIView *filterButtonView;
    NSDate *selectedDate;
    int tagIndex;
    BOOL isShowExpGoal;
    BOOL isArranging;
    ...
    NSArray* days;
    NSArray* months;
    NSMutableArray *mainGoalArr;
}

@end

@implementation VisionGoalActionViewController
@synthesize isFromBuddy;
...
- (void)viewDidLoad {
    ...
    mainGoalArr = [NSMutableArray new];
    expRowNumber = -1;
    isShowExpGoal = false;
    isArranging = false;
    ...
    activeDayIndex = 0;
    selectedColor = [UIColor colorWithRed:(228/255.0f) green:(39/255.0f) blue:(160/255.0f) alpha:1.0f];
    grayColor = [UIColor colorWithRed:(88/255.0f) green:(88/255.0f) blue:(88/255.0f) alpha:1.0f];
    filterView.hidden = true;
    for (UIButton *btn in resultCollectionBtn) {
        btn.layer.cornerRadius = 15;
        btn.layer.masksToBounds = YES;
        if (btn.tag == 121) {
            btn.layer.borderColor = squadMainColor.CGColor;
    ...
    selectDateBtn.layer.cornerRadius = 15;
    selectDateBtn.layer.masksToBounds = YES;
...
//    [myGoalTableView addObserver:self forKeyPath:@"contentSize" options:0 context:nil];
    ...
    if (![Utility isEmptyCheck:[defaults objectForKey:@"selectedGoalHeader"]]) {
        selectedHeader = [[defaults objectForKey:@"selectedGoalHeader"]mutableCopy];
    }else{
        selectedHeader = [NSMutableArray new];
    }
    if (![Utility isEmptyCheck:[defaults objectForKey:@"selectedActionRow"]]) {
        selectedRow = [[defaults objectForKey:@"selectedActionRow"]mutableCopy];
    }else{
...
//    [myGoalTableView removeObserver:self forKeyPath:@"contentSize"];
    [defaults setObject:selectedHeader forKey:@"selectedGoalHeader"];
    [defaults setObject:selectedRow forKey:@"selectedActionRow"];
...
//            }
            ...
            controller.goalName = @"ADD GOAL";//goalNameTextField.text;
...
-(IBAction)toggleAction:(UIButton *)sender{
    ...
    NSDictionary *dict = [[NSDictionary alloc]initWithObjectsAndKeys:[NSNumber numberWithInt:[sender.accessibilityHint intValue]],@"ReminderID", nil];
...
- (IBAction)saveButtonPressed:(UIButton *)sender {
    if (isArranging) {
        ...
        for (int i = 0; i<activeGoalArray.count; i++) {
            ...
            [temp setObject:[NSNumber numberWithInt:i+1] forKey:@"GoalOrder"];
...
-(IBAction)goalDateExpandCollapseButton:(UIButton *)sender{
    if (sender.tag == 1) {
        goalExpandButton.selected = !goalExpandButton.selected;
        if (goalExpandButton.isSelected) {
            for (UIView *goalview in goalCollection) {
                goalview.hidden = true;
            }
            goalArrButton.selected = true;
        }else{
            for (UIView *goalview in goalCollection) {
                goalview.hidden = false;
            }
            goalArrButton.selected = false;
        }
        
    }else{
        dateExpandButton.selected = !dateExpandButton.selected;
        if (dateExpandButton.isSelected) {
            for (UIView *dateview in dateCollection) {
                dateview.hidden = true;
            }
            dateArrButton.selected = true;
        }else{
            for (UIView *dateview in dateCollection) {
                dateview.hidden = false;
            }
            dateArrButton.selected = false;
        }
    }
}
-(IBAction)goalTickPressed:(UIButton*)sender{
    for (UIButton *btn in goalDatetickBtn) {
        if (btn == sender) {
            btn.selected = true;
...
-(IBAction)showResultBuutonPressed:(id)sender{
...
//        self->expRowNumber = self->activeGoalArray.count;
    ...
    if (![Utility isEmptyCheck:self->goalListArray]) {
        ...
        if (tagIndex == 10 || tagIndex == 13) {
            goalListArray = [mainGoalArr mutableCopy];
        }else if (tagIndex == 11){
            goalListArray = [activeGoalArray mutableCopy];
        }else if (tagIndex == 12){
            NSArray *expFilteredArray = [mainGoalArr filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"IsExpired == 1"]];
            goalListArray = [expFilteredArray mutableCopy];
        }else if (tagIndex == 14){
            NSDateFormatter *format = [[NSDateFormatter alloc]init];
            [format setDateFormat:@"YYYY-MM-dd"];
            NSLog(@"%@",[NSPredicate predicateWithFormat:@"(CreatedAt = %@)",[format stringFromDate:[NSDate date]]]);
            NSArray *todayArray = [mainGoalArr filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(CreatedAt = %@)",[format stringFromDate:[NSDate date]]]];
            goalListArray = [todayArray mutableCopy];
        }else{
            if (![Utility isEmptyCheck:selectedDate]) {
                NSDateFormatter *format = [[NSDateFormatter alloc]init];
                [format setDateFormat:@"YYYY-MM-dd"];
                NSLog(@"%@",[NSPredicate predicateWithFormat:@"(CreatedAt = %@)",[format stringFromDate:selectedDate]]);

                NSArray *todayArray = [mainGoalArr filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(CreatedAt = %@)",[format stringFromDate:selectedDate]]];
                goalListArray = [todayArray mutableCopy];
            }
        }
        filterView.hidden = true;
        if (![Utility isEmptyCheck:self->goalListArray]) {
            ...
            [self->myGoalTableView reloadData];
            [self->myGoalTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:NSNotFound inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:NO];
...
-(IBAction)clearAllPressed:(id)sender{
    ...
    [self showResultBuutonPressed:0];
}
...
//                                                                             NSDictionary *dict = [[NSUserDefaults standardUserDefaults] dictionaryRepresentation];
                                                                             ...
                                                                             if (![Utility isEmptyCheck:self->goalListArray] && self->goalListArray.count>0) {
                                                                                 self->filterButtonView.hidden = false;
                                                                                 self->myGoalTableView.hidden = false;
                                                                                 ...
                                                                                 self->mainGoalArr = [self->goalListArray mutableCopy];
                                                                                 self->tagIndex = 10;
                                                                                 for (UIButton *btn in self->goalDatetickBtn) {
                                                                                     if (btn.tag == 10) {
                                                                                         btn.selected = true;
                                                                                     }else{
                                                                                         btn.selected = false;
                                                                                 ...
                                                                                 self->filterButtonView.hidden = true;
                                                                                 self->myGoalTableView.hidden = true;
                                                                             }
                                                                         }
...
- (void) setUpReminderFor:(NSString *) section {            //ah ln1
    ...
    if ([section caseInsensitiveCompare:@"Goal"] == NSOrderedSame) {
        ...
        for (int i = 0; i < activeGoalArray.count; i++) {
            NSDictionary *dict = [[NSDictionary alloc] initWithDictionary:[activeGoalArray objectAtIndex:i]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == myGoalTableView) {
        ...
        if (![Utility isEmptyCheck:goalListArray]) {
            ...
            if ([[dailyDateformatter dateFromString:startDateStr] isEarlierThanOrEqualTo:[dailyDateformatter dateFromString:[dailyDateformatter stringFromDate:[NSDate date]]]] && [[dailyDateformatter dateFromString:[dailyDateformatter stringFromDate:[NSDate date]]] isEarlierThanOrEqualTo:[dailyDateformatter dateFromString:dateStr]]) {
                //active
                add = 1;
            }else{
                //expired
            ...
            NSLog(@"%@",[NSNumber numberWithInt:[[[goalListArray objectAtIndex:section] objectForKey:@"id"]intValue]]);
...
//                [sectionHeaderView.swapView setBackgroundColor:[UIColor colorWithRed:(0/255.0f) green:(100/255.0f) blue:(255/255.0f) alpha:0.7f]];
//            }else{
...
//            }
        ...
        if (expRowNumber == section) {
            ...
            NSDictionary *attrDict = @{
                                       NSFontAttributeName : [UIFont fontWithName:@"Oswald-Light" size:17.0],
        ...
        sectionHeaderView.goalHeaderLabel.text = [[[goalListArray objectAtIndex:section] objectForKey:@"Name"]capitalizedString];
        [sectionHeaderView.goalHeaderLabel setTextColor:grayColor];//selectedColor
        sectionHeaderView.goalHeaderImageView.image = [UIImage imageNamed:@"ok_uncheck.png"];
//        [sectionHeaderView.goalHeaderImageView sd_setImageWithURL:[NSURL URLWithString:[[goalListArray objectAtIndex:section] objectForKey:@"Picture"]] placeholderImage:[UIImage imageNamed:@"you_have_got.png"] options:SDWebImageScaleDownLargeImages];
        ...
        if ([selectedHeader containsObject:[NSNumber numberWithInt:[[[goalListArray objectAtIndex:section] objectForKey:@"id"]intValue]]]){
            [sectionHeaderView.expandCollapseButton setImage:[UIImage imageNamed:@"up_arrow_mbHQ.png"] forState:UIControlStateNormal];
        } else {
            [sectionHeaderView.expandCollapseButton setImage:[UIImage imageNamed:@"down_arrow_mbHQ.png"] forState:UIControlStateNormal];
        }
        /*
...
//        [myView setBackgroundColor:[UIColor colorWithRed:88/255.0f green:88/255.0f blue:88/255.0f alpha:1.0f]];
        [myView setBackgroundColor:[Utility colorWithHexString:@"f1f2f2"]];//[Utility colorWithHexString:@"333333"]
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == myGoalTableView) {
        ...
        if (![Utility isEmptyCheck:goalDetailsArr] && indexPath.row < goalDetailsArr.count) {
            //data
//            NSArray *goalDetailsArr = [[goalListArray objectAtIndex:indexPath.section] objectForKey:@"GoalActionDetails"];
            cell.goalLabel.text = [[[goalDetailsArr objectAtIndex:indexPath.row] objectForKey:@"Name"]capitalizedString];
//            cell.goalLabel.textColor = [UIColor colorWithRed:88/255.0f green:88/255.0f blue:88/255.0f alpha:1.0f];
            ...
            if (![[[goalDetailsArr objectAtIndex:indexPath.row] objectForKey:@"LastReminderStatus"] boolValue]) {
                ...
                placeholderImage = [UIImage imageNamed:@"tick_mbhq.png"];
            }
            ...
            if ([selectedRow containsObject:modRow]){//if (indexPath.row == selectedRow) {
                ...
                [cell.expCollRowButton setImage:[UIImage imageNamed:@"up_arrow_mbHQ.png"] forState:UIControlStateNormal];
            }else{
                cell.tempHistoryView.hidden = true;
                [cell.expCollRowButton setImage:[UIImage imageNamed:@"down_arrow_mbHQ.png"] forState:UIControlStateNormal];
            }
...
//            NSDateComponents *components;
            if ([[df dateFromString:dateStr] isEarlierThanOrEqualTo:[df dateFromString:[df stringFromDate:[NSDate date]]]] && [[df dateFromString:[df stringFromDate:[NSDate date]]] isEarlierThanOrEqualTo:[df dateFromString:dueDate]]) {
                //active
...
//                        tickButton.accessibilityHint = [NSString stringWithFormat:@"%d",i];//date
                        ...
                        if (empty) {
                            tickButton.tag = i;
                            tickButton.accessibilityHint = [NSString stringWithFormat:@"%@",[currHistory objectForKey:@"goal_mindset_reminder_master_id"]];
                ...
                cell.completedLabel.text = [NSString stringWithFormat:@"%d OF %d",total,remTotal];// 7 remTotal
            }
...
//            cell.completedLabel.text = [NSString stringWithFormat:@"%lu OF %lu",(unsigned long)days,(unsigned long)totalDays];
            ...
            //add button
            if(![Utility isEmptyCheck:goalDetailsArr]){
                cell.addActionLabel.text = @"Add new action here";
            }else{
                ...
                NSDictionary *attrDict = @{
                                           NSFontAttributeName : [UIFont fontWithName:@"Raleway-Light" size:15.0],
...
//                self->newActionLabel.attributedText = text;
                cell.addActionLabel.attributedText = text;
            }
...
-(void)didSelectDate:(NSDate *)date{
    ...
    if (date) {
        ...
        [selectDateBtn setTitle:dateString forState:UIControlStateNormal];
        tagIndex = 0;
    }
}
@end
