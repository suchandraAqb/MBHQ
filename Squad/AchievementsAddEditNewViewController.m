...
//
//  AchievementsAddEditNewViewController.m
//  Squad
//
...
@interface AchievementsAddEditNewViewController (){
    ...
    IBOutlet UILabel *uploadImageLabel;
    ...
    IBOutlet UILabel *userEntryLabel;
    IBOutlet UITextField *userEntryTextField;
    IBOutlet UITextView *userEntryTextView;
    
    IBOutlet UIView *threeOptionView;
    IBOutlet UIView *threeOptionMenuView;
    IBOutlet UIButton *threeOptionViewCrossButton;
    IBOutlet UIButton *outerEditButton;
    IBOutlet UIButton *innerEditButton;
    IBOutlet UIButton *shareButton;
    
    IBOutlet UIView *userEntryDisplayView;
    IBOutlet UIView *partitionterDisplayView;
    bool editingMode;
    IBOutlet UIButton *saveStackViewButton;
    IBOutlet UIView *shareView;
}

@end

@implementation AchievementsAddEditNewViewController
...
//    [addEditImage setTitle:@"Change Image" forState:UIControlStateNormal];
    deleteImageButton.hidden = false;
     NSString *uploadPictureImgFileName = [achievementsData objectForKey:@"UploadPictureImgFileName"];
                       
                       if([uploadPictureImgFileName isEqual: [NSNull null]]){
            imageView.hidden = true;
            lableView.hidden = true;
        uploadImageLabel.hidden = true;
        addEditImage.titleLabel.text = @"";
//        uploadImageLabel.hidden = false;
        }else{
...
- (void)cropViewController:(PECropViewController *)controller didFinishCroppingImage:(UIImage *)croppedImage transform:(CGAffineTransform)transform cropRect:(CGRect)cropRect
{
    [controller dismissViewControllerAnimated:YES completion:^{
        self->isChanged = YES;
        self->saveStackViewButton.selected = true;
        [saveStackViewButton setImage:[UIImage imageNamed:@"checked_active.png"] forState:UIControlStateNormal];
...
//    [addEditImage setTitle:@"Change Image" forState:UIControlStateNormal];
     NSString *uploadPictureImgFileName = [achievementsData objectForKey:@"UploadPictureImgFileName"];
    NSLog(@"uploadPictureImgFileName====>%@",uploadPictureImgFileName);
    NSLog(@"");
//                       if([uploadPictureImgFileName isEqual: [NSNull null]]){
    if(croppedImage == nil){
            imageView.hidden = true;
...
-(void) saveDataMultiPart {
    if (Utility.reachable) {
        if (![Utility isEmptyCheck:achievementTitle.text] && achievementTitle.text.length > 0 && ![achievementTitle.text isEqualToString:@"Add Achievement"]) {
            if (![Utility isEmptyCheck:userEntryLabel.text]) {
                NSString *temp = [NSString stringWithFormat:@"%@%@",achievementTitle.text,userEntryLabel.text];
                NSLog(@"temp====>%@",temp);
                achievementTitle.text = temp;
            }
        ...
        if (![Utility isEmptyCheck:selectedDate]) {
            ...
            [achievementsData setObject:date forKey:@"CreatedAt"];
            [dateFormatter setDateFormat:@"yyyy-MM-dd"];
            NSString *datestring=[dateFormatter stringFromDate:dateFromCurrent];
            [achievementsData setObject:datestring forKey:@"CreatedAtString"];
        }else{
...
-(void)prepareView{
    ...
    [outerEditButton setImage:[UIImage imageNamed:@"editing_inactive.png"] forState:UIControlStateNormal];
    if (![Utility isEmptyCheck:achievementsData]) {
        NSLog(@"achievementsData====>%@",achievementsData);
        NSLog(@"");
        NSString *createdDate = [achievementsData objectForKey:@"CreatedAt"];
        ...
        NSString *title = [listItems[0] stringByAppendingString:@":"];
        achievementTitle.text = ![Utility isEmptyCheck:title] ? title : @"Add Achievement";
        achievementTitle.font = [UIFont boldSystemFontOfSize:22];
              
        
        NSString *userEntry;
        for (int i = 1; i<listItems.count; i++) {
            if (i<listItems.count-1) {
                userEntry = [listItems[i] stringByAppendingString:@" "];
            }else{
                userEntry = [listItems[i] stringByAppendingString:@""];
            }
        }
        if(![Utility isEmptyCheck:userEntry]){
            userEntryLabel.text = userEntry;
            userEntryLabel.font = [UIFont systemFontOfSize:18];
        }else{
//            userEntryLabel.text = @"Nothing";
//            userEntryLabel.font = [UIFont systemFontOfSize:18];
            userEntryDisplayView.hidden = true;
            partitionterDisplayView.hidden = true;
        }
        if([achievementTitle.text containsString:@"Grateful For"]){
...
//        achievementDetails.accessibilityHint =![Utility isEmptyCheck:description] ? description: @"";
        ...
        if(![Utility isEmptyCheck:[achievementsData objectForKey:@"PictureDevicePath"]]){
            ...
            lableView.hidden = true;
            uploadImageLabel.hidden = true;
            achievementImageView.layer.cornerRadius = 15;
            achievementImageView.layer.masksToBounds = YES;
...
//                                                       [self->addEditImage setTitle:@"CHANGE IMAGE" forState:UIControlStateNormal];
                                                      NSString *uploadPictureImgFileName = [self->achievementsData objectForKey:@"UploadPictureImgFileName"];
                       
                       if([uploadPictureImgFileName isEqual: [NSNull null]]){
                           self->imageView.hidden = true;
                        self->lableView.hidden = true;
                        self->uploadImageLabel.hidden = true;
                           self->addEditImage.titleLabel.text = @"";
//        uploadImageLabel.hidden = false;;
                                                          }else{
...
//        uploadImageLabel.hidden = false;;
    ...
    [threeOptionView addGestureRecognizer:tap];
}
...
-(IBAction)takeImageButtonPressed:(id)sender{
UIAlertController* alert = [UIAlertController
                            alertControllerWithTitle:nil      //  Must be "nil", otherwise a blank title area will appear above our two buttons
                            message:nil
                            preferredStyle:UIAlertControllerStyleActionSheet];
...
- (IBAction)saveButonPressed:(UIButton *)sender {
    saveStackViewButton.selected = true;
    userEntryLabel.text = userEntryTextView.text;
...
-(IBAction)outerEditButtonPressed:(id)sender{
    threeOptionView.hidden = false;
}
-(IBAction)threeOptionViewCrossButtonPressed:(id)sender{
    threeOptionView.hidden = true;
}
-(IBAction)innerEditButtonPressed:(id)sender{
    editingMode = true;
    threeOptionView.hidden = true;
    bottomView.hidden = false;
    userEntryLabel.hidden = true;
    userEntryTextView.hidden = false;
    userEntryDisplayView.hidden = false;
    userEntryTextView.text = userEntryLabel.text;
    [outerEditButton setImage:[UIImage imageNamed:@"editing_active.png"] forState:UIControlStateNormal];
    userEntryTextView.layer.cornerRadius = 15;
    userEntryTextView.layer.masksToBounds = true;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    saveStackViewButton.selected = false;
...
//    saveButton.layer.masksToBounds = YES;
    ...
    threeOptionMenuView.layer.cornerRadius = 15;
    threeOptionMenuView.layer.masksToBounds = true;
