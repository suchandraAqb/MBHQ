...
- (BOOL)isMaximumSelectionLimitReached
{
    ...
    if (minimumNumberOfSelection <= self.imagePickerController.maximumNumberOfSelection) {
        BOOL isReturn = (self.imagePickerController.maximumNumberOfSelection <= self.imagePickerController.selectedAssets.count);
        if (isReturn) {
            [Utility showToastInsideView:self.view WithMessage:@"Maximum number of photo is selected"];
//            [Utility msg:@"Maximum number of photo is selected" title:nil controller:self haveToPop:NO];
        }
        return isReturn;
    }
