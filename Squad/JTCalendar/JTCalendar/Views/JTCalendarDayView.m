...
- (void)commonInit
{
    ...
    {
        _dayImage = [UIImageView new];
        [self addSubview:_dayImage];
        
        _dayImage.backgroundColor = [UIColor clearColor];
        _dayImage.image = nil;
        _dayImage.hidden = YES;
        
        _dayImage.layer.rasterizationScale = [UIScreen mainScreen].scale;
        _dayImage.layer.shouldRasterize = YES;
    }
    {
...
- (void)layoutSubviews
{
    ...
    _dayImage.frame = CGRectMake(0, 0, sizeCircle, sizeCircle);
    _dayImage.center = CGPointMake(self.frame.size.width / 2., self.frame.size.height / 2.);
    _dayImage.layer.cornerRadius = sizeCircle / 2.;
