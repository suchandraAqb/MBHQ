...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    backCamera = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo]; //AVCaptureDevice.defaultDevice(withMediaType: AVMediaTypeVideo)
    [_flashButton setImage:[UIImage imageNamed:@"flash_off_icon"] forState:UIControlStateNormal];
    [backCamera lockForConfiguration:nil];
    backCamera.flashMode = AVCaptureFlashModeOff;
    [backCamera unlockForConfiguration];

    [_timerButton setImage:[UIImage imageNamed:@"off_icon"] forState:UIControlStateNormal];
...
#pragma mark Helpers
- (void) captureCounter {
    if (captureTimeInterval == 0) {
        [self takePicture];
        [captureTimer invalidate];
        _timerLabel.hidden = true;
        [_timerButton setUserInteractionEnabled:YES];
    } else {
        _timerLabel.hidden = false;
        _timerLabel.text = [NSString stringWithFormat:@"%d",captureTimeInterval];
        captureTimeInterval--;
        [_timerButton setUserInteractionEnabled:NO];
    }
}
...
- (void)handleImage:(UIImage *)image
{
    [self dismissViewControllerAnimated:YES completion:^{
        [_cameraDelegate didFinishedTakingPhoto:image];
    }];
    
//    ImageViewerViewController *viewController = [self.storyboard instantiateViewControllerWithIdentifier:@"ImageViewer"];   //[[ImageViewerViewController alloc] initWithImage:image];
...
-(IBAction)showHideOverlayButtonTapped:(UIButton *)sender {
    if (sender.isSelected) {
        sender.selected = false;
        _overlayImageView.hidden = true;
    } else {
        sender.selected = true;
        _overlayImageView.hidden = false;
    }
}

- (IBAction)timerTapped:(id)sender {
    if (captureTimeInterval == 0) {
        captureTimeInterval = 5;
        [_timerButton setImage:[UIImage imageNamed:@"5sec"] forState:UIControlStateNormal];
    } else if (captureTimeInterval == 5) {
        captureTimeInterval = 10;
        [_timerButton setImage:[UIImage imageNamed:@"10sec"] forState:UIControlStateNormal];
    } else if (captureTimeInterval == 10) {
        captureTimeInterval = 0;
        [_timerButton setImage:[UIImage imageNamed:@"off_icon"] forState:UIControlStateNormal];
    }
}
            ...
            if (backCamera.flashMode == AVCaptureFlashModeOff) {
                if ([backCamera isFlashModeSupported:AVCaptureFlashModeOn]) {
                    @try {
                        ...
                        [backCamera unlockForConfiguration];
                        flashModeStr = @"on";
                        [_flashButton setImage:[UIImage imageNamed:@"flash_on_icon"] forState:UIControlStateNormal];
                    } @catch (NSException *exception) {
                        NSLog(@"error -> %@",exception);
                    }
                }
            }else if (backCamera.flashMode == AVCaptureFlashModeOn) {
                if ([backCamera isFlashModeSupported:AVCaptureFlashModeAuto]) {
                    @try {
                        [backCamera lockForConfiguration:nil];
                        backCamera.flashMode = AVCaptureFlashModeAuto;
                        [backCamera unlockForConfiguration];
                        flashModeStr = @"auto";
                        [_flashButton setImage:[UIImage imageNamed:@"flash_auto"] forState:UIControlStateNormal];
                    } @catch (NSException *exception) {
                        NSLog(@"error -> %@",exception);
                    }
                }
            }else if (backCamera.flashMode == AVCaptureFlashModeAuto) {
...
- (IBAction)takePhotoButtonWasTouched:(UIButton *)sender
{
    if (captureTimeInterval == 0) {
        ...
        captureTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(captureCounter) userInfo:nil repeats:YES];
